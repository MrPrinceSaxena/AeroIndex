...
Source 1 — Air India direct (airindia.com / official GDS stream)
...
class AirIndiaDirectConnector(BaseConnector):
    """
    Live high-resilience collector for Air India economy flight quotes across Indian domestic routes.
    """
    ...
    async def fetch_all(
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Multi-selector JavaScript extraction script
                extraction_js = """
                (() => {
                    const fares = [];
                    ...
                    for (const el of elements) {
                        ...
                        const isAI = /Air India|AI[-\\s]\\d+|AirIndia/i.test(text);
                    ...
                    if (fares.length === 0) {
                        const ariaEls = document.querySelectorAll("[aria-label*='Air India'], [aria-label*='AirIndia']");
                    ...
                    for route in routes:
                        ...
                        try:
                            ...
                            if extracted_prices:
                                ...
                                print(f"[{self.SOURCE_NAME}] No direct Air India quotes found for {route} on {date_str}.")
